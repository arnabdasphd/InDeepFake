...
import argparse
import sys
sys.path.append('/home/aritra/manohar/fsgan-master')
...
def add_audio_to_existing_output(target_path, output_path):
    # Define the temporary output path
    temp_output_path = os.path.join(os.path.dirname(output_path), 'temp_output.mp4')
    
    try:
        # Extract the audio from the target video and add it to the output video
        (
            ffmpeg
            .input(output_path)
            .output(temp_output_path, vcodec='copy', acodec='aac', audio_bitrate='192k', **{'map': '0:v', 'map': f'{target_path}:a'})
            .run(overwrite_output=True)
        )
        
        # Replace the original output file with the new output file
...
class FaceSwapping(VideoProcessBase):
    ...
    def __call__(self, source_path, target_path, output_path=None, select_source='longest', select_target='longest',
        ...
        add_audio_to_existing_output(source_path, target_path, output_path)
