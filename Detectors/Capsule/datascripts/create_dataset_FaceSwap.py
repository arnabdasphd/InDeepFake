...
parser.add_argument('--type', default ='FaceSwap')
parser.add_argument('--list', default ='path_to_FaceForensicsPP/splits/train.json')
parser.add_argument('--output', default ='path_to_output/3_faceswap')
...
def extract_face_videos(filename, compress):
    ...
    while (success_cap_vid and success_cap_mask_ref):
        ...
        if image_cropped is not None:
            cv2.imwrite(os.path.join(opt.output, compress + '_' + filename + "_%d_faceswap.jpg" % count), image_cropped)
            count += 1
