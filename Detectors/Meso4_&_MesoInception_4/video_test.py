...
def load_and_preprocess_image(image):
    img = cv2.resize(image, (IMGWIDTH, IMGWIDTH))
...
def process_video_directory(directory, model, frame_rate=1):
    video_predictions = {}
    for filename in os.listdir(directory):
        if filename.endswith(".mp4") or filename.endswith(".avi"):
            ...
            if frames.size == 0:
                print("No frames found in video {}".format(filename))
                continue
            predictions = model.predict(frames)
            average_prediction = np.mean(predictions)
            video_predictions[filename] = average_prediction
...
if __name__ == "__main__":
    ...
    # Process videos and get average predictions
    video_predictions = process_video_directory(video_directory, model)

    # Print the average predictions for each video
    for video, prediction in video_predictions.items():
        print("Video: {}, Average Prediction: {}".format(video, prediction))
